...
HR2_list <- list.files("./Data/", pattern="(HR2)", full.names = TRUE)
...
HR2_1 <- HR2 %>% select(c(Date.Time.Offset, Model, Receiver.ID, Full.ID, Detection.Type)) %>% dplyr::filter(Detection.Type == 3) %>% mutate(Model ='HR2') %>% 
